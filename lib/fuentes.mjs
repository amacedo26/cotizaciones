...
import { aNumero, interpretarBcu, interpretarSwissquote, interpretarDolarApi,
         interpretarBluelytics, calcularBrecha, calcularDxy, contrastarArgentina } from './parseo.mjs';
...
// --- variación contra la medición anterior -----------------------------------
// Ninguna de estas fuentes publica el valor previo, así que la variación se
// calcula contra la corrida anterior propia. Vive acá y no en la página porque
// depende del histórico, que la página recibe ya resuelto.
...
export function aplicarVariaciones(foto, punto, anterior) {
  for (const [clave, donde] of DONDE_VA_LA_VARIACION) {
    const destino = donde(foto);
    if (!destino) continue;
    const ahora = punto[clave];
    const antes = anterior?.[clave];
    destino.variacion = (typeof ahora === 'number' && typeof antes === 'number' && antes !== 0)
      ? +(((ahora - antes) / antes) * 100).toFixed(2)
      : null;
  }
  // La variación del momento anterior también sirve para saber contra qué se
  // está comparando: sin esto, un salto grande tras un corte parece un error.
  if (anterior?.t) foto.comparadoCon = anterior.t;
  return foto;
}
