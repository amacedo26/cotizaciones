...
// Las unidades de cuenta se piden al mismo servicio que el dólar, así que
// necesitan su propia banda: la del dólar rechazaría a las dos.
// Referencias de setiembre de 2026: UI ≈ 6,63 y UR ≈ 1.923.
export const UI_MIN = 1;
export const UI_MAX = 1_000;
export const UR_MIN = 100;
export const UR_MAX = 1_000_000;
...
// La banda es un parámetro porque el mismo servicio sirve el dólar (≈40), la
// UI (≈6,6) y la UR (≈1.900): un solo rango plausible no puede cubrirlas.
export function interpretarBcu(xml, banda = { min: UYU_MIN, max: UYU_MAX }) {
  ...
  if (promedio < banda.min || promedio > banda.max) {
...
// La pizarra del BROU. El sitio del banco es un portal que arma la tabla en el
// navegador: el HTML que llega por fetch no trae ningún número, así que se lee
// del espejo de dolarapi. La fila que interesa es la que el BROU rotula sólo
// "Dólar" (no eBROU ni cable), que acá viene como moneda USD.
export function interpretarBrou(json) {
  if (!Array.isArray(json)) return { estado: 'error', error: 'respuesta que no es una lista' };
  const fila = json.find((f) => (f?.moneda || '').toUpperCase() === 'USD');
  if (!fila) return { estado: 'error', error: 'la lista no trae el dólar' };

  const compra = aNumero(fila.compra);
  const venta = aNumero(fila.venta);
  if (compra === null && venta === null) {
    return { estado: 'error', error: 'el dólar vino sin compra ni venta' };
  }
  const promedio = compra !== null && venta !== null
    ? +((compra + venta) / 2).toFixed(4)
    : (compra ?? venta);

  if (promedio < UYU_MIN || promedio > UYU_MAX) {
    return { estado: 'revisar', error: `valor fuera de rango plausible: ${promedio}`, compra, venta };
  }
  return {
    estado: 'ok',
    nombre: fila.nombre ?? 'Dólar',
    compra, venta, promedio,
    fecha: fila.fechaActualizacion ?? null,
  };
}
